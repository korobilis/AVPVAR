function [y,x,Z1,Z2,Z3,Z4,Z5,Z6,theta_t,beta_t,sigma_t] = outtvpsv_reg_dgp2(T,p,param)
...
for t = 1:T+100
    if t == 1
        theta_t(t,:) = mu +  0.999*(theta0 - mu) + (1/(T^(2/4))).*trnd(3,1,p);
    else
        theta_t(t,:) = mu +  0.999*(theta_t(t-1,:) - mu) + (1/(T^(2/4))).*trnd(3,1,p);
    end
end
...
% Informative Drivers
Z4 = [ones(T,1)  trnd(3,T,20)  ];
Z5 = [ones(T,1)  trnd(3,T,40)  ];
Z6 = [ones(T,1)  trnd(3,T,60)  ];
