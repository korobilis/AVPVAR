function [y,x,Z1,Z2,Z3,Z4,Z5,Z6,theta_t,beta_t,sigma_t] = outtvpsv_reg_dgp3(T,p,param)
...
It = zeros(T,1); 
h1 = [round(5*T/8):round(7*T/8)];           % Outlier observation
w = 2 + 0.2*rand(length(h1),1);        % Outlier strength
It(h1,1) = w;
h2 = [round(4*T/8):round(6*T/8)];           % Outlier observation
w = 2 + 0.2*rand(length(h2),1);        % Outlier strength
It(h2,2) = w;
h3 = [round(2*T/8):round(5*T/8)];           % Outlier observation
w = 2 + 0.2*rand(length(h3),1);        % Outlier strength
It(h3,3) = w;
h4 = [round(3*T/8):round(7*T/8)];           % Outlier observation
