function   [Y,Z,T,M,data,dataZ,dates,varnames] = load_data_Instruments(select,selectZ,tcode,tcodeZ,standard,standardZ)
%% Function to load data for VAR
% read in VAR series
A          = readtable('USData_Updated.xlsx',"Sheet","Variables");
% read in Instruments
B          = readtable('Z.xlsx',"Sheet","predictordata");
% Merge tables (keeps all dates)
C = outerjoin(A, B, 'Keys', 'Dates', 'MergeKeys', true);
...
[~, index] = ismember(selectZ,varnames); 
dataZ      = All_data(:,index); %Instruments data
