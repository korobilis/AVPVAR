%% Plots of series: Y, Z and cumsum(Z)
clear; clc; close all;
...
financial_crisis_start = datetime('2008-03-01');
financial_crisis_end   = datetime('2009-06-30');
covid_start            = datetime('2020-01-01');
covid_end              = datetime('2020-06-01');
...
legend(hY,{'RGDP','HICP'},'Location','southwest','FontSize',8,'Box','on');
...
ylim([-10 10]); grid on;
set(gca,'GridAlpha',0.3,'GridColor',[0.8 0.8 0.8],'FontSize',9,'Box','on','LineWidth',0.8,'XTickLabel',[]);
legend(hZ,{'POILU','PCOMU','COMPR','YWR'},'Location','southwest','FontSize',8,'Box','on');
...
legend(hZc,{'POILU','PCOMU','COMPR','YWR'},'Location','southwest','FontSize',8,'Box','on');
title('Panel C: Cumulative Drivers','FontSize',10,'FontWeight','bold');
hold off;

%% Export 
exportgraphics(gcf,'Figures\plots_EA.pdf','ContentType','vector','BackgroundColor','none');
