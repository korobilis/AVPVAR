...
burnin            = 1000;%2 * ceil(0.1*MCMCdraws);    % burn in
...
fcstYdraws         = NaN(N,fcstNhorizons, Ndraws, MCMCdraws/thin); % see reshape near end of script
fcstSVdraws        = NaN(N,fcstNhorizons, Ndraws, MCMCdraws/thin);
fcstSVoutliers     = NaN(N,fcstNhorizons, Ndraws, MCMCdraws/thin);
fcstLogscoreDraws  = NaN(Ndraws, MCMCdraws/thin); % see reshape near end of script
fcstLogscoreDrawsRB = NaN(Ndraws, MCMCdraws/thin); % see reshape near end of script

yhatdraws   = NaN(N,fcstNhorizons, MCMCdraws/thin);
...
PAI_all     = NaN(K,N,MCMCdraws/thin); 
PHI_all     = NaN(N*(N-1)/2+N,MCMCdraws/thin);
invA_all    = NaN(N,N,MCMCdraws/thin); 
sqrtht_all  = NaN(N,T,MCMCdraws/thin);

SVOprob_all  = NaN(N, MCMCdraws/thin);   
SVOscale_all = NaN(N, T, MCMCdraws/thin);
...
m = 0;
while m < MCMCreps %MCMCreps % using while, not for loop to allow going back in MCMC chain
    ...
    if m > burnin && mod(m-burnin, thin) == 0;
        
        thisdraw = thisdraw+1; %m-burnin;
        % STORE DRAWS
...
fcstYdraws          = reshape(fcstYdraws, N, fcstNhorizons,fcstNdraws/thin);
fcstLogscoreDraws   = reshape(fcstLogscoreDraws, fcstNdraws/thin, 1);
fcstLogscoreDrawsRB = reshape(fcstLogscoreDrawsRB, fcstNdraws/thin, 1);

fcstYhatRB     = mean(yhatdraws,3);
fcstYhat       = mean(fcstYdraws,3);

fcstSVdraws    = reshape(fcstSVdraws, N, fcstNhorizons, fcstNdraws/thin);
fcstSVoutliers = reshape(fcstSVoutliers, N, fcstNhorizons, fcstNdraws/thin);
